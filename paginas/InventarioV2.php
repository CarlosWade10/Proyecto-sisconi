  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"> <i class="fa-solid fa-user fa-lg"></i> AGREGAR </h5>
...
-->
  ...
  <form action="../controlador/controlador_RegistraInve.php" method="post" id="formulario">
  <div class="mb-3">
    <label for="nombreProve2" class="form-label">Nombre del producto:</label>
    <input type="text" class="form-control" id="nombreProve2" placeholder="agregar nombre del producto" require name="Nombre" onkeypress="return SoloLetras(event);">
  </div>


  <div class="mb-3">
    <label for="NumeroProve2" class="form-label">cantidad:</label>
    <input type="number" class="form-control" id="NumeroProve2" placeholder="agrega la cantidad" require name="Numero" min="0">
  </div>

  <div class="mb-3">
    <label for="NumeroProve3" class="form-label">precio de compra:</label>
    <input type="number" class="form-control" id="NumeroProve3" placeholder="agrega precio de compra" require name="Numero2" min="0">
  </div>   

  <div class="mb-3">
    <label for="NumeroProve4" class="form-label">precio de venta:</label>
    <input type="number" class="form-control" id="NumeroProve4" placeholder="agrega precio de venta" require name="Numero3" min="0">
  </div>
...
<div class="modal fade" id="ModalEditarProveedor" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fa-sharp fa-regular fa-pen-to-square fa-lg"></i> Editar </h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
...
<form action="../controlador/controlador_EditarInve.php" method="post" id="formulario2">

<input type="hidden" id="IDProveedor" name="IDProveedor" value="0">

   <div class="mb-3">
    <label for="NombreProve" class="form-label">Nombre del producto:</label>
  ...
  <div class="mb-3">
    <label for="NumeroProve" class="form-label">cantidad:</label>
    <input type="number" class="form-control" id="NumeroProve" placeholder="agrega la cantidad" require name="NumeroE">
  </div>

  <div class="mb-3">
    <label for="NumeroProve2" class="form-label">precio de compra:</label>
    <input type="number" class="form-control precompra" id="NumeroProve2" placeholder="agrega el precio de compra" require name="NumeroE2">
  </div>

  <div class="mb-3">
    <label for="NumeroProve3" class="form-label">precio de venta:</label>
    <input type="number" class="form-control preventa" id="NumeroProve3" placeholder="agrega el precio de venta" require name="NumeroE3">
  </div>
...
var listar = function(){
 var table = $('#myTable').DataTable({
  "ajax":{
   "method": "POST",
   "url": "../controlador/listarInve.php" 
  },
  "columns":[
   {"data":"ID"}, 
   {"data":"NombreProducto"},  
...
$(tbody).on("click", "button.editar",function(){
  var data = table.row($(this).parents("tr")).data();
   var ID = $('#IDProveedor').val(data.ID),
    Nombre = $('#NombreProve').val(data.NombreProducto),
    Numero = $('#NumeroProve').val(data.existencia),
    FechaRegistro = $('.precompra').val(data.PrecioCompra),
    Estado = $('.preventa').val(data.PrecioVenta);
});
}
...
$(tbody).on("click", "button.eliminar",function(){
  ...
  Numero = data['existencia'],
  FechaRegistro = data['PrecioCompra'];
  Estado = data['PrecioVenta'];
   console.log('El ID de la fila seleccionada es: ' + id +" "+Nombre);
   alertaEliminar(id, Nombre, Numero, FechaRegistro, Estado);
});
}

function alertaEliminar(ID, nombre, NP, FR, E){
 Swal.fire({
  title: 'estás seguro que deseas eliminar?',
  //text: "ID del proveedor: "+ ID + " Nombre del proveedor: "+ nombre,
  html: '<b>ID del producto:</b> '+ID +'<br>'+
        '<b>Nombre:</b> '+ nombre +'<br>'+
        '<b>cantidad:</b> '+ NP +'<br>'+        
        '<b>precio de compra:</b> '+ FR +'<br>'+        
        '<b>precio de venta:</b> '+ E +'<br>',        
  icon: 'warning',
...
function eliminar_php(codigo){
 ...
 $.ajax({
  data: parametros,
  url: "../controlador/controlador_eliminarInve.php",
  type: "POST",
  beforeSend: function(){},
  success: function(){
   Swal.fire({
  ...
  timer: 1500
    ...
    window.location = "InventarioV2.php"
